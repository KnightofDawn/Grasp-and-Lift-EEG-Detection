#Windows Size changed from 1024 to 1280
#Subsampling is changed from 16 to 4
#Filter and take in data of 7-30 Hz


#TODO:Experimenting 
#Add dropout layers to avoid overfitting ------> self.add(DropOutLayer(keep=0.5))
#Increase number of epochs, also change rho and epsilon
...
from  neon.layers import DropOutLayer
...
class GalData(Dataset):
    ...
    def prep(self, data):
        # TODO: Preprocessing code added.
	filtered_data = butter_bandpass_filter(data, 7, 30, 250, order=5)
	if self.mean is None:
            self.mean = filtered_data.mean()
            self.std = filtered_data.std()
        filtered_data -= self.mean
        filtered_data /= self.std
...
def run(subj):
    ...
    sumpreds = None
    winlist = [1024] if validate else [768, 1024, 1280, 1536]       #Windows Size changed from 1024 to 1280
    for winsize in winlist:
        ds.setwin(winsize=winsize, subsample=4)                    #Subsampling is changed from 16 to 4
...
if __name__ == '__main__':
    ...
    if validate:
        ...
        subm.to_csv('convsubtwo.csv', index_label='id', float_format='%.4f')
    print('Done.')
